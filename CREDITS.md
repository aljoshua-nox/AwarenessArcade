...
| Numbered character pack (`pack/pack/1.png`…) | `assets/art/characters/pack/` | ⚠️ unidentified — no bundled license found | ⚠️ unknown | ⚠️ needs source lookup (unreferenced by the game) |
| Street passer-by A — the 4x4 walk sheet `district_exterior.gd` loads as `NPC_A_TEXTURE` | `assets/art/characters/24by24ModernRPGGuy.png` | OVFudj — *Modern RPG Guy*, [OpenGameArt](https://opengameart.org/content/modern-rpg-guy) (2018; identified 2026-09-20 by its filename, which is the submission's attachment name; 1.4 KB, 96x96, matches) | **CC0** | ✅ |
...
2. Identify the remaining ⚠️ art: the `office/` floor and wall tiles (in the game) and the
   numbered `pack/` character sheets (not referenced by anything — could simply be
   removed). ~~`24by24ModernRPGGuy.png`, the first street NPC, had no row at all~~ —
