...
| Portraits (`teddy`, `joel`, `trish`, `carmen`, `dennis`, `bea`, `rowena`, `gus`) | `assets/art/portraits/` | **AI-generated**, produced by the team (2026-09-12) from the briefs in the workspace's `future-cast.md`; 1254x1254 RGB. Added to the repo 2026-09-15 ahead of the characters they are for; `teddy` became Teodoro Villanueva (`interview_case_007`), `trish` Patricia Lim (`008`) and `bea` Bea Santiago (`009`) the same day; the other five are not yet referenced by a case | No third-party license — generated for this project. Same terms as the row above | ✅ |
